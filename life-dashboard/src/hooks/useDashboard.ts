...
export const useDashboard = () => {
  ...
  // Calculate progress for an area (including children)
  const calculateAreaProgress = useCallback((area: Area): number => {
    const trackerProgress = area.trackers.map(t => {
      if (t.type === 'boolean') return t.value ? 100 : 0;
      if (t.target) return Math.min((t.value / t.target) * 100, 100);
      if (t.max) return (t.value / t.max) * 100;
      return 50; // Default if no target
    });
