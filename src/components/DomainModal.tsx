...
function MetricFields({
  ...
  kind: 'binary' | 'progress' | 'stages';
  setKind: (k: 'binary' | 'progress' | 'stages') => void;
  binaryValue: 0 | 1;
  ...
  stagesText: string;
  setStagesText: (v: string) => void;
}) {
  const stageCount = stagesText.split(',').map(s => s.trim()).filter(Boolean).length;
  return (
    <>
      <div>
      ...
      {kind === 'stages' && (
        <>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Stages (comma-separated)</label>
            <input
              type="text"
              value={stagesText}
...
export function DomainModal({
    ...
    if (isEdit && area) {
      ...
      if (area.metric) {
        ...
        if (area.metric.type === 'stages') {
          setCurrentIndex(area.metric.currentIndex);
          setStagesText(area.metric.stages.join(', '));
        }
      } else {
        setKind('binary');
        setBinaryValue(0);
  ...
  const buildMetric = (): DomainMetric | null => {
    ...
    return { type: 'stages', currentIndex: Math.min(currentIndex, stages.length - 1), stages };
  };
